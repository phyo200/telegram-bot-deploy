...
async def update_file_content(path, content, sha, message):
    ...
    try:
        ...
        print(f"Error updating Github content: {e}")
        return None
...
def check_key_expiration(expiration_time):
    try:
        ...
        mm, hh, dd, MM, yyyy = map(int, expiry.split('-'))
...
async def scan(message):
    ...
    mode = args[1]
    chat_id = message.chat.id
    if not approve.get(chat_id, False) or chat_id not in user_data:
        await bot.reply_to(message, "/scan ကိုအသုံးမပြုမီ /key ကိုအရင်ပြုလုပ်ပေးပါ။")
        return
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, message=None, progress_msg=None):
    ...
    # Progress updating timer throttle
    ...
    try:
        while True:
            ...
            # Throttle Telegram message edit to avoid hitting 429 Flood Limits (Edit max once per 3s)
            now_time = time.monotonic()
            if now_time - progress_timers.get(chat_id, 0) >= 3.0 or (total and checked == total):
        ...
        limited_messages.pop(chat_id, None)
        limited_texts.pop(chat_id, None)
        progress_timers.pop(chat_id, None)
...
async def perform_check(session_url, code, chat_id, scan_id=None, recheck=False, message=None):
    ...
    # Thread-safe message updating lock
        ...
        async with lock:
            ...
            if message:
                try:
                    ...
                    print(f"Success Message Edit Error: {e}")
        ...
        async with lock:
            ...
            if message:
                try:
                    ...
                    print(f"Limited Message Edit Error: {e}")
